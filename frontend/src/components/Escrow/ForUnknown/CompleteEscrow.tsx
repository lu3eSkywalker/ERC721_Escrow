...
const CompleteEscrow = () => {
  ...
  const ABI = [
    "function completeEscrowForUnknownParties(uint256) public payable",
  ];
  ...
  async function completeEscrow() {
    if (window.ethereum) {
      try {
        ...
        const newValue = ethers.parseUnits(enterEth, 0);

        const approveUser = await contract.completeEscrowForUnknownParties(
          tokenId,
          {
            value: enterEth,
          }
